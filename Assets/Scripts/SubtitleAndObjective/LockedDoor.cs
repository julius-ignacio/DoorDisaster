...
public class LockedDoor : MonoBehaviour
{
    ...
    void OnTriggerStay(Collider other)
    {
        if (other.CompareTag("Player") && !doorUnlocked)
        {
            if (Input.GetKeyDown(KeyCode.E))
            {
                if (!hasKey)
                {
                    // Step 1: play collapsing sound immediately
                    AudioManager.PlayCatchFire();
    ...
    private IEnumerator ShowLockedSequence()
    {
        ...
        AudioManager.PlayObjectiveUpdate();
    ...
    private void UnlockDoor()
    {
        ...
        isDoorOpen = true;
        AudioManager.PlayObjectiveUpdate();
        AudioManager.PlayEscapeSuccess();
